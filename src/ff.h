...
typedef struct FF_GAME_BOOT_STATE {uint32 sp,fp,gp,ra,heap_begin,heap_bytes;} FF_GAME_BOOT_STATE;
...
GDB_CALL void ff_game_frame_prefix_80014DF8_stage0(void);
...
static __inline uint32 ff_u32(uint32 a) { return *(uint32*)ff_ptr(a,4); }
static __inline sint16 ff_s16(uint32 a) { return *(sint16*)ff_ptr(a,2); }
static __inline void ff_w32(uint32 a,uint32 v) { *(uint32*)ff_ptr(a,4)=v; }
static __inline void ff_w8(uint32 a,uint8 v) { *(uint8*)ff_ptr(a,1)=v; }
static __inline void ff_w16(uint32 a,uint16 v) { *(uint16*)ff_ptr(a,2)=v; }
#endif
